...
public class SpaceTree<T> extends AbstractDimensionalMap<T> {
    ...
    private void addAllFromNode(Node nodeToAddFrom, Node nodeToAddTo) {
        if (nodeToAddFrom != null) {
            if (nodeToAddFrom.center == null) {
                // If it's a leaf just add to node all from bucket
                if (nodeToAddTo == null) {
                    for (NodeEntry<T> nodeEntry : nodeToAddFrom.nodeBucket) {
                        add(nodeEntry.position, nodeEntry.value);
                    }
                } else {
                    for (NodeEntry<T> nodeEntry : nodeToAddFrom.nodeBucket) {
                        addToNode(nodeEntry.position, nodeToAddTo, nodeEntry.value);
                    }
                }
            } else {
                if (nodeToAddTo == null) {
    ...
    private static final class TreeSearch<T> {
        ...
        public void addEntry(Entry<T> entry) {
            ...
            if (size > maxCapacity) {
                ...
                SortedSet<Entry<T>> entriesAtThisDistance = results.get(maxDistanceInResults);
                Entry<T> last = entriesAtThisDistance.last();
                if (last != null)
                    entriesAtThisDistance.remove(last);
